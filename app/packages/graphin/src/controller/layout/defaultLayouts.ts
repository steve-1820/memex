...
import ConcentricLayout, {
  ConcentricOption,
} from '../../layout/basic/concentric';
  ...
  return [
    ...
    },
    {
      ...
      layout: (data: Data, options: LayoutOption): { data: Data } => {
        ...
        return {
          data: dagreLayout(data, {
            ...defaultOptions,
            ...options,
          } as DagreLayoutOption),
        };
      },
    },
    {
    ...
    },
    {
      ...
      layout: (
      ...
      ): { data: Data; forceSimulation: ForceSimulation } => {
        const defaultOptions = {
          data,
          /** 前置布局，默认为 concentric */
          preset: {
          ...
          maxSpeed: 1000,
          /** 最大迭代数 */
          MaxIterations: 100000, // 240, // 1000000次/(1000/60) = 60000s = 1min
          /** 是否开启动画 */
          animation: true,
        };
        ...
        // 处理 前置布局后的数据
        if (presetName === 'force') {
          presetData = TweakLayout(presetData, options as ForceLayoutOptions)
            .data;
        } else {
          ...
          presetData = presetLayout?.layout(
            data,
            presetOptions as ForceLayoutOptions
          ).data as Data;
        }
