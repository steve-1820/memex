...
function makePolyEdge(
  ...
  const polyEdges = _.flatMap(polyGroups, (group) => {
    let distance = isEven(group.length) ? -20 : -40;
    return group
      .map((edge) => {
        return {
          edge,
      ...
      .map(({ edge, point }, index) => {
        ...
        if (!source || !target)
          throw new Error('Invalid Edge, Cannot Find Source/Target Node');

        if (isEven(group.length) && isEven(index)) distance += 40;
        if (isOdd(group.length) && (isOdd(index) || index === 0))
          distance += 40;
        const polyEdge = {
          ...edge,
          shape: 'PolyEdge',
        ...
        polyEdge.poly = {
          distance: isEven(index) ? distance : -distance,
        };
        return polyEdge;
      });
  });
  ...
  let transformedEdges: Data['edges'] = edges
    .filter((edge) => {
      ...
      if (!edge.data) {
        // eslint-disable-next-line no-console
        console.warn(
          `edge requires an data field，this '${JSON.stringify(
            edge
          )}' edge will be automatically filtered`
        );
        return false;
...
function checkNodes(
  ...
  const graphinNodes = nodes
    .filter((node) => {
      ...
      if (!id) {
        console.warn(
          `node requires an id，this '${JSON.stringify(
            node
          )}' node will be automatically filtered`
        );
        return false;
      }
      if (!node.data) {
        console.warn(
          `node requires an data field，this '${JSON.stringify(
            node
          )}' node will be automatically filtered`
        );
        return false;
