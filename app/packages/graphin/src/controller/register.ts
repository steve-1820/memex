...
import RegisterCurveEdge from '../shape/graph-studio/CurveEdge';
...
import RegisterCustomNode from '../shape/graph-studio/CustomNode';
...
const graphinRegister = (props: GraphinProps) => {
  ...
  // props.register 处理
  const {
    nodeShape = dummyRegister,
    edgeShape = dummyRegister,
    behavior = dummyRegister,
  } = register;
  ...
  const {
    icon = dummyIcon,
    nodeShape: ExNodeShape = dummyExtend,
    marker: ExMarker = dummyExtend,
  } = extend;
