...
class Graph extends React.PureComponent<GraphinProps, GraphinState> {
  ...
  componentDidMount() {
    ...
    const {
      data: newData,
      forceSimulation,
    } = layoutController(this.getContext(), { data });
  ...
  componentDidUpdate(prevProps: GraphinProps) {
    ...
    if (isDataChange || isLayoutChange) {
      ...
      const { data, forceSimulation } = layoutController(this.getContext(), {
        data: currentData,
        prevProps,
      });
      this.forceSimulation = forceSimulation!;
  ...
  renderChildren = () => {
    ...
    if (
      ...
      console.error(
        'Please do not wrap components inside dom element or Fragment when using Graphin'
      );
      return children;
    }
