...
export default (g6: any) => {
  g6.registerNode('HexagonNode', {
    ...
    setState(name: EnumNodeAndEdgeStatus, value: string, node: INode) {
      ...
      const hexagonBorder = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'hexagon-border');
      ...
      const hexagonLabel = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'hexagon-label');
      const hexagonChildrenGroup = container
        .get('children')
        .find(
          (item: Shape.Base) => item.attr().id === 'hexagon-children-group'
        );
      ...
      if (hexagonChildrenIcon)
        hexagonChildrenIcon.attr(targetAttrs.childrenIcon);
    },
    getAnchorPoints() {
