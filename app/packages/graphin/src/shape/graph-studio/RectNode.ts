...
export default (g6: any) => {
  g6.registerNode('RectNode', {
    draw(cfg: G6Node, group: Group) {
      ...
      inner.addShape('text', {
        attrs: {
          ...
          y: innerSize / 2,
          text: iconFont(
            cfg.style?.icon || cfg.data.type || '',
            cfg.style?.fontFamily! || DEFAULT_ICON_FONT_FAMILY
          ),
          fontSize: 20,
          textAlign: 'center',
          textBaseline: 'middle',
    ...
    setState(name: EnumNodeAndEdgeStatus, value: string, node: INode) {
      ...
      const rectBorder = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'rect-border');
      const rectSelected = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'rect-selected');
      const rectInnerGroup = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'rect-inner-group');
      const rectInner = rectInnerGroup
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'rect-inner');
      const rectIcon = rectInnerGroup
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'rect-icon');
      const rectLabel = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'rect-label');
