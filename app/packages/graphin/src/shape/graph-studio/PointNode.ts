...
export default (g6: any) => {
  g6.registerNode('PointNode', {
    ...
    setState(name: EnumNodeAndEdgeStatus, value: string, node: INode) {
      ...
      const floor = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'point-floor');
      const selected = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'point-selected');
      const text = container
        .get('children')
        .find((item: Shape.Base) => item.attr().id === 'point-label');
