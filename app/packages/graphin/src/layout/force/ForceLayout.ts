...
class ForceLayout {
  ...
  init = () => {
    ...
    this.nodes.forEach((node) => {
      ...
      this.nodePoints.set(
        node.id,
        new Point(vec, String(node.id), node.data, mass)
      );
    });
  ...
  updateCoulombsLaw = () => {
    ...
    for (let i = 0; i < len; i++) {
      for (let j = i + 1; j < len; j++) {
        ...
        const v = (this.nodePoints.get(iNode.id) as Point).p.subtract(
          this.nodePoints.get(jNode.id).p
        );
  ...
  updateHookesLaw = () => {
    this.edges.forEach((edge) => {
      ...
      spring.source.updateAcc(
        direction.scalarMultip(-this.props.stiffness * displacement)
      );
      spring.target.updateAcc(
        direction.scalarMultip(this.props.stiffness * displacement)
      );
    });
  };
  ...
  attractToCentre = () => {
    ...
    this.nodes.forEach((node) => {
      // 默认的向心力指向画布中心
      const degree = (node.data &&
        node.data.layout &&
        node.data.layout.degree) as number;
      ...
      const { leaf, single, others, center } = {
        ...defaultRadio,
        ...this.props.centripetalOptions,
      };
